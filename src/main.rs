...
async fn main() {
    ...
    for url in urls {
        ...
        if parser.streams_info.is_empty() {
            continue;
        }
        ...
        if count == 1 {
            ...
            match config.all_channels {
                Some(ref s) => {
                    ...
                    let diff_file =
                        output_dir.join(format!("{}_diff{}", fs.to_str().unwrap(), ext));
                    let original_contents = read_to_string(&of).unwrap_or_default();
                    ...
                    for j in v {
                        let title = j["title"].as_str().unwrap_or("NO_TITLE");
                        if title.is_empty() {
                            println!("No title for stream: {j:?}");
                        } else {
                            writeln!(output, "{title}").expect("Error");
                        }
                    }
                    ...
                    for change in cdiff.iter_all_changes() {
                        ...
                        if sign != " " {
                            write!(diff_output, "{sign} {change}")
                                .expect("Failed to write diff info");
                            changes += 1;
                        }
                    }
                    if changes > 0 {
